...
//        }
//        
        self.player = [AVPlayer playerWithPlayerItem:_playerItme];
...
-(void)getVolumeVolue
{
    ...
    [_volumeViewSlider setValue:1.0f animated:NO];
...
//        [_player replaceCurrentItemWithPlayerItem:self.playerItme];
//    });
    [self.player replaceCurrentItemWithPlayerItem:self.playerItme];
    // AVPlayer播放完成通知
//    self.player = [AVPlayer playerWithPlayerItem:self.playerItme];
    if([[UIDevice currentDevice] systemVersion].intValue>=10){
        //      增加下面这行可以解决iOS10兼容性问题了
        self.player.automaticallyWaitsToMinimizeStalling = NO;
    }
...
-(void)tapOnce:(UITapGestureRecognizer *)gesture{
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:@"playerViewsHide" object:nil];
...
{
    self.maskView.videoSlider.value += value/15000 ;
}
